...
# the nth row of matrix A by the elements in the mth column of matrix B, then take the sum 
...
# works with a 2x2 but need to adapt to larger grids

def matrix_mult(a, b)
  width = a[0].length
  height = a.length
#   create a list of numbers that we will divide up into proper rows
  num_list = []
  new_matrix = []

#   calculating all values in first row
  1.upto(width) do |i|
    first = a[0][0] * b[0][i - 1] + a[0][1] * b[1][i - 1]
    num_list << first
  end

#   calculating all values in second row
  1.upto(height) do |i|
    second = a[1][0] * b[0][i - 1] + a[1][1] * b[1][i - 1]
    num_list << second
  end

#   dividing the number list into sub arrays that make up the rows to be added back
  num_list.each_slice(width) { |array_row| new_matrix << array_row }
